...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, if( (imm & 0x20 ) == 0) R(rd) =  (int32_t)src1 >> imm);//仅当 shamt[5]=0 时合法
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beqz   , B, if(src1 == src2) s->dnpc = s->pc + imm); // beq条件判断跳转
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , r, R(rd) = (uint32_t)src1 << src2);//空位补零
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , r, R(rd) = src1 & src2);
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", snez   , r, R(rd) = (src1 < src2) ? 1 : 0);//sltu
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , r, R(rd) = ((int64_t)(int32_t) src1 * (int64_t)(int32_t) src2) >> 32);//M  ?????
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , r, R(rd) = (int32_t)src1 >> (int32_t)src2);//用原数最高位（符号位）填充,就是补码的算术右移,未考虑第五位为位移数
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , r, R(rd) = (uint32_t)src1 >> src2);//空位补零,未考虑第五位为位移数
