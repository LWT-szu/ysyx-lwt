module LSU (
    ...
    input reg lsu_respValid,//存储器响应有效（数据已准备好）
    output reg lsu_reqValid,//访存请求有效 发给存储器（MEM）
